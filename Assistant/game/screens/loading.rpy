    ...
    vbox:
        ...
        if app.error:
            if app.loading_recovery == "server_config":
                ...
                textbutton "Reset server settings" action Function(app.reset_server_config) sensitive (not app.recovery_busy)
                ...
                textbutton "Reset Assistant preferences" action Function(app.reset_assistant_config) sensitive (not app.recovery_busy)
            text "[app.error]" color "#ff8d8d" style "app_text"
            textbutton "Retry" action Function(app.start_loading) sensitive (not app.recovery_busy)
        elif app.starting or app.recovery_busy:
