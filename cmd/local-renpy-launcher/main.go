// Command local-renpy-launcher owns the exported Ren'Py client pack lifecycle.
//
// It starts the bundled server, verifies that the instance on the fixed
...
func main() {
	if err := run(); err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "Local Ren'Py launcher: %v\n", err)
		os.Exit(1)
	}
}

func run() error {
	pairingLink := pairingLinkArgument(os.Args[1:])
	root, err := packRoot()
	if err != nil {
		return err
	}
	...
	renpyArgs := []string{}
	if pairingLink != "" {
		renpyArgs = append(renpyArgs, pairingLink)
	}
	renpy := exec.Command(renpyPath, renpyArgs...)
	renpy.Dir = root
...
func pairingLinkArgument(args []string) string {
	for _, arg := range args {
		if strings.HasPrefix(strings.ToLower(arg), "amnesia://") {
			return arg
		}
	}
	return ""
	...
	for {
		...
		case <-ticker.C:
			...
			if health.Status != "ok" || health.InstanceID != expectedInstance {
				return fmt.Errorf("port %s is occupied by another server", serverPort)
			}
			return nil
		}
	}
