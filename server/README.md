...
| server | Loopback HTTP + `~/.amnesia-agent-server/config.json` + per-turn sessions |
...
```text
amnesia-agent-server --host 127.0.0.1 --port 8765
```
...
protects against malicious web pages without requiring a frontend-embedded
authentication secret.

This server exposes the kernel's unrestricted local shell tool. Keep local mode
on loopback and run it only on a trusted machine.

## Remote mode

Remote mode is an explicit opt-in and requires a public HTTPS origin for pairing
links:

```text
amnesia-agent-server --remote \
  --public-url https://agent.example.com \
  --host 127.0.0.1 --port 8765
```

Put Caddy/nginx in front of the loopback listener and expose only HTTPS publicly.
A direct non-loopback bind additionally requires `--tls-cert` and `--tls-key`.
Remote mode:

- prints one `amnesia://pair` QR/link at startup and rotates it after ten minutes;
- invalidates a pairing code immediately after its first use;
- exposes `POST /v1/pair` for the one-time exchange;
- requires a per-device `Authorization: Bearer ...` token on every other API route;
- disables `/v1/shutdown`;
- stores device-token hashes under `~/.amnesia-agent-server/device-tokens.json`.

The pairing QR is an enrollment secret. Anyone who can see the current server
terminal or retained service logs can pair a device. Run the process as a
non-root user and do not expose port 8765 directly to the Internet. The agent's
shell tool remains unrestricted, so remote mode is single-owner only and is not
...
Defaults live as importable Python constants in `amnesia_agent_server.config`
(`DEFAULT_MODEL`, `DEFAULT_COMMAND_TIMEOUT_SECONDS`, … / `default_config_dict()`).
They are persisted at `~/.amnesia-agent-server/config.json` (separate from
...
`amnesia-agent-server` / `__main__` entrypoint (which wires
...
6. **`workspace_path` removed from config** — no longer in `LoadedConfig` /
   ...
   `~/.amnesia-agent-server/config.json` or load fails loud. Frontends
...
**Corrupt config** — Fix or delete `~/.amnesia-agent-server/config.json`,
...
custom host. Start via `amnesia-agent-server` for shutdown support.
