"""Persistent configuration owned by the server."""
...
DEFAULT_CONFIG_DIR = Path.home() / ".amnesia-agent-server"
LEGACY_CONFIG_DIR = Path.home() / ".amnesia-agent-local-server"
...
def _resolve_root(root: str | os.PathLike[str] | None) -> Path:
    if root is None:
        # Preserve existing installations after the package rename. New users
        # use the new directory; an existing legacy directory remains usable
        # until the operator explicitly migrates or removes it.
        if not DEFAULT_CONFIG_DIR.exists() and LEGACY_CONFIG_DIR.exists():
            return LEGACY_CONFIG_DIR.resolve(strict=False)
...
class ConfigStore:
    """Load, save, and reset ``~/.amnesia-agent-server/config.json``."""
