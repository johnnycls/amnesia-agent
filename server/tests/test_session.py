...
from amnesia_agent_server.routes.turn import stream_sse
...
class SessionTests(unittest.TestCase):
    ...
    def test_cancel_active_turn_acloses_and_releases_busy(self) -> None:
        async def run() -> None:
            with tempfile.TemporaryDirectory() as directory:
                ...
                _write_config(store)
                manager = SessionManager(store)
                with patch("amnesia_agent_server.session.KernelSession", ClosingFakeSession):
                    events_agen = manager.start_turn("hello")
    ...
    def test_active_turn_blocks_config_and_workspace_writes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            _write_config(store)
            manager = SessionManager(store)
            from amnesia_agent_server.session import resolved_workspace_key
    ...
    def test_different_paths_can_run_turns_in_parallel(self) -> None:
        async def run() -> None:
            ...
            with tempfile.TemporaryDirectory() as directory:
                ...
                _write_config(store)
                manager = SessionManager(store)
                with patch("amnesia_agent_server.session.KernelSession", HoldingFakeSession):
