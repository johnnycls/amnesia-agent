...
class WorkspaceApiTests(unittest.TestCase):
    def test_workspace_lifecycle_endpoints(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            client = TestClient(create_app(store))
            with patch(
                "amnesia_agent_server.session.KernelSession.check_workspace",
                return_value=False,
            ) as check:
            ...
            with patch(
                "amnesia_agent_server.session.KernelSession.setup_or_repair_workspace"
            ) as setup:
            ...
            with patch(
                "amnesia_agent_server.session.KernelSession.create_workspace"
            ) as create:
            ...
            with patch(
                "amnesia_agent_server.session.KernelSession.create_or_reset_workspace"
            ) as reset:
    ...
    def test_update_history_endpoint(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("amnesia_agent_server.session.KernelSession", FakeSession):
                response = client.put(
            ...
            with patch("amnesia_agent_server.session.KernelSession", TrackingSession):
                client.put(
                    "/v1/workspace/history",
                    json={
                        "messages": [{"role": "user", "content": "hi"}],
