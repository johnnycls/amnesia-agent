...
_CONFIG_KEYS: Final[frozenset[str]] = frozenset(
    {"selected_character_id", "language", "server_url", "server_token"}
)
...
class PersistentAssistantConfigStore:
    ...
    def load(self) -> AssistantConfig:
        ...
        server_token = getattr(persistent, "server_token", None)
        if server_token == "":
            server_token = None
        if server_token is not None and not isinstance(server_token, str):
            raise ConfigError("persistent.server_token must be a string or null")
        try:
    ...
    def save(self, config: AssistantConfig) -> None:
        ...
        persistent.server_token = config.server_token
    ...
    def set_selected_character(self, character_id: str) -> AssistantConfig:
        ...
        updated = AssistantConfig(
            character_id.strip(), current.language, current.server_url, current.server_token
        )
        self.save(updated)
        return updated
    ...
    def set_server_credentials(self, server_url: str, server_token: str) -> AssistantConfig:
        try:
            canonical = normalize_server_url(server_url)
        except ServerUrlError as error:
...
class AssistantConfigStore:
    ...
    def _parse(self, raw: Any) -> AssistantConfig:
        ...
        if server_token is not None and not isinstance(server_token, str):
            raise ConfigError(
                f"server_token must be a string or null, got {type(server_token).__name__}",
                path=str(self.path),
            )
        try:
            server_url = normalize_server_url(server_url)
