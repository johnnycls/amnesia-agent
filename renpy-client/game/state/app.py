...
from api.pairing import PairingLinkError, PairingPayload, parse_pairing_link
...
class AppState:
    ...
    def start_loading(self) -> None:
        ...
        local_url = DEFAULT_SERVER_URL
        persisted_url = assistant_config.server_url
        persisted_token = assistant_config.server_token
        pending_pairing = self.pending_pairing

        def configure(url: str, token: str | None) -> None:
            self.client.set_base_url(url)
            setter = getattr(self.client, "set_auth_token", None)
            if setter is not None:
                setter(token)

        def check_health(url: str, token: str | None) -> None:
            configure(url, token)
        ...
        def load_server_config() -> dict[str, Any]:
            return self.client.request_json("GET", "/v1/config")
        ...
        def work() -> None:
            ...
            try:
                try:
                    check_health(local_url, None)
                except ApiError:
                    if pending_pairing is not None:
                        pairing_result = self.client.pair(
                            pending_pairing.server_url,
                            pending_pairing.code,
                            device_name="assistant",
                        )
                        paired_url = normalize_server_url(str(pairing_result["server_url"]))
                        paired_token = str(pairing_result["device_token"])
                        check_health(paired_url, paired_token)
                        config = load_server_config()
                        self.config_store.set_server_credentials(paired_url, paired_token)
                        self.pending_pairing = None
                        active_url, active_token = paired_url, paired_token
                    else:
                        check_health(persisted_url, persisted_token)
                        config = load_server_config()
                        active_url, active_token = persisted_url, persisted_token
                else:
                    try:
                        config = load_server_config()
            ...
            except (ApiError, CharacterError, ConfigError, OSError, KeyError, TypeError) as error:
    ...
    def receive_deep_link(self, value: str) -> None:
        """Queue an OS-delivered pairing link for the next failed-local check."""
        try:
            self.pending_pairing = parse_pairing_link(value)
        except PairingLinkError as error:
            self.status = f"Invalid pairing link: {error}"
            self.error = str(error)
            self._refresh()
            return
        self.status = "Pairing link received. It will be used if the server is unavailable."
        self._refresh()
    ...
    def _loading_ok(
        ...
        self.server_url = active_url or assistant_config.server_url
        self.server_token = active_token if active_url is not None else assistant_config.server_token
        self._set_client_credentials(self.server_url, self.server_token)
    ...
    def _set_client_credentials(self, server_url: str, token: str | None) -> None:
    ...
    def _server_connection_ok(self, server_url: str, operation_id: int) -> None:
        ...
        self._set_client_credentials(server_url, None)
        self._end_operation(operation_id)
    ...
    def reset_server_url(self) -> None:
        self.status = "The server URL is selected through QR pairing."
        self.error = ""
        self._refresh()

    def reset_server_config(self) -> None:
        """Reset corrupt server settings, then resume normal startup."""
        if self.starting or self.operation is not None or self.ready:
            return
