...
class Client:
    ...
    def __init__(self, base_url: str = DEFAULT_SERVER_URL, auth_token: str | None = None) -> None:
    ...
    def set_base_url(self, base_url: str) -> str:
        """Validate and apply a new server origin."""
        self._base_url = normalize_server_url(base_url)
        return self._base_url

    def set_auth_token(self, auth_token: str | None) -> None:
        if auth_token is not None and not isinstance(auth_token, str):
            raise ValueError("Server auth token must be a string or null")
        self._auth_token = auth_token or None
    ...
    def request_json(
        ...
        try:
            ...
            raise ApiError(_http_error_detail(error)) from error
        except (URLError, OSError) as error:
            raise ApiError(f"Server request failed: {error}") from error
        if not raw.strip():
            return {}
        try:
    ...
    def pair(
        ...
        pairing_client = Client(server_url)
        return pairing_client.request_json(
            "POST",
            "/v1/pair",
    ...
    def stream_turn(
        ...
        """POST /v1/turn and consume its SSE stream in a background thread."""
        handle = TurnHandle()
        payload: dict[str, Any] = {
            "text": text,
            "response_format": ASSISTANT_STAGE if response_format is None else response_format,
            ...
            try:
                ...
                if not handle.cancelled:
                    ...
                    if isinstance(error, HTTPError):
                        ...
                        invoke(on_error, TurnTimeoutError("Connection timed out while waiting for the provider."))
                    else:
...
def validate_turn_event(event: dict[str, Any]) -> dict[str, Any]:
    ...
    if event_type not in _TURN_EVENT_TYPES:
        raise SseError(f"Server returned unknown turn event: {event_type!r}")
...
def _http_error_detail(error: HTTPError) -> str:
    try:
        ...
        pass
    return f"Server HTTP {error.code}: {error.reason}"
